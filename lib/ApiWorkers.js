...
_.each(_.values(ApiResources), function(resource) {
  ...
  worker.onmessage = function(e) {
    if (e.data.type === ApiWorkerMessages.REST_RESPONSE) {
      ...
      logger.info(logPrefix + 'Received notification message - ' + util.inspect(notification));
...
module.exports.doRequest = function(resource,
  ...
  logger.info(lp + 'request to resource - ' + resource.fullPath() + ', method - ' + method + ", options - " + util.inspect(options));
  ...
  logger.info(lp + 'Created msg - ' + util.inspect(msg));
